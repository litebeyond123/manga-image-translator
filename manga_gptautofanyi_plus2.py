...
def your_chatgpt_api_call(conn, input_text):
    ...
    # get summary
    cur = conn.cursor()
    # Ensure the manga_summary table exists and has an initial value
    cur.execute('''CREATE TABLE IF NOT EXISTS manga_summary (summary TEXT)''')
    cur.execute('SELECT COUNT(*) FROM manga_summary')
    count = cur.fetchone()[0]
    if count == 0:
        cur.execute('INSERT INTO manga_summary (summary) VALUES ("")')
    ...
    for page in pages:
        ...
        for sentence in sentences:
            ...
            json_obj[page_key]["@IndependentDialogue " + str(sentence[
                                                                 0])] = "[No need for complete sentences and translate separately to Chinese without merging] " + \
                                                                        sentence[1].strip().replace(" ",
                                                                                                    "") + " [stop translate]"
...
while True:
    ...
    try:
        ...
        if len(word_array) < batch_size:
            print(
                "The current number of untranslated pages is less than batch_size and automatically adjusted to:" + str(
                    len(word_array)))
